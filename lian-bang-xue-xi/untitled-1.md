...
_安全聚合的权衡_ 对于安全聚合，研究以下类似的问题将是非常有趣的。考虑一次有n个用户的FL轮，假设用户i的值是xi。用户i对xi应用算法A\(·\)得到yi= A\(xi\);在这里，A\(·\)可以被认为是一个压缩和私有化方案。使用安全聚合作为黑盒，服务提供商观察¯y =P iA\(xi\)，并通过计算ˆ¯x = g\(¯y\)来估算xi的真实和¯x。理想情况下，我们希望设计A\(·\)、g\(·\)时，能够最小化估算¯x时的误差;形式上，我们希望解决最优化问题，Akg\(P iA\(xi\)\)−P ixik，其中k.k可以是' 1或' 2范数。当然，无需对g\(.\)和A\(·\)施加任何约束，我们总是可以选择它们为恒等函数和get0error。然而，A\(·\)必须满足两个约束条件:\(1\)A\(·\)应该输出B位\(可以认为是每个用户的通信成本\)，\(2\)$y=\sum\_iA\(x\_i\)$应该是¯x = pixi的\(ε， δ\)-DP版本。因此，我们感兴趣的基本问题是确定最优算法A，在满足固定通信预算的同时，在聚合时实现DP。从不同的角度来看这个问题，对于固定的n, B， ε和δ，我们希望实现的最小的l1或l2误差是什么？我们注意到Agarwal等人的工作，\[9\]提供了一个基于均匀量化和二项式噪声加法的候选算法A。然而，最近\[256\]提出了另一种解决方案，它涉及旋转、缩放和离散数据，然后在进行模块化裁剪和安全聚合之前添加离散高斯噪声。而独立的和离散高斯函数不是离散高斯,作者表明,它是足够接近和现在紧DP担保和实验结果,证明他们的解决方案能够实现类似的准确性通过连续高斯噪声中央DP 16每值\(或更少\)的精度。然而，目前尚不清楚这种方法是否实现了最佳的通信、隐私和准确性权衡。因此，在上述约束条件下，推导出误差的下界并匹配误差的上界是非常重要的。
...
教育部分应衡量和提高用户对正在使用的隐私技术\(如第4.2条\)和数据使用细节的理解。对于涉及联邦学习的应用程序，这还应该包括联邦学习的解释，以及将向服务器发送的确切数据。一旦该研究的教育成分证实了典型用户可以有意义地理解私人学习过程提供的隐私保证，那么研究人员就可以开始偏好诱导。这可以发生在行为实验室，大规模的现场实验，或小的焦点小组。应当谨慎地确保提供有关其偏好数据的个人既了解足够的情况以提供高质量的数据，又能代表目标人口。
...
* 保护用户数据的隐私需要考虑计算数据的哪些功能以及如何执行计算\(特别是谁可以看到/影响中间结果\)。\(4.2节\)
  * 解决“什么”的技术包括数据最小化和差分隐私。\(4.2.2 4.3.2节\)。如何最好地适应不同的隐私审计和私有化技术以适应现实世界的部署仍然是一个重要的开放式挑战，包括由多个独立的参与者在重叠人口上训练大量的机器学习模型，使用随着时间变化的数据，在现实世界的不确定性环境中，如客户可用性，所有这些都不会迅速耗尽隐私预算，同时保持高实用性。
...
机器学习模型经常会表现出出人意料和意想不到的行为。当这种行为导致对用户产生不良影响的模式时，我们可以根据一些标准将这种模式归类为“不公平”。例如，如果具有相似特征的人得到的结果截然不同，那么这就违反了个人公平的标准\[169\]。如果某些敏感群体\(种族、性别等\)得到不同的结果模式——例如不同的假阴性率——这可能违反人口统计学公平的各种标准，参见调查\[51,349\]。反事实公平的标准要求用户在考虑所有因果相关的途径后，获得与其他群体\(种族、性别等\)成员相同的待遇\[287\]。
...
明确获取人口统计信息\(种族、性别等\)对许多现有的公平标准\(包括6.1节中讨论的标准\)至关重要。然而，当个别敏感属性不可用时，联邦学习经常部署的上下文也会引起对公平性的考虑。例如，当开发个性化语言模型或开发公平的医学图像分类器而不知道有关个人的任何其他人口统计信息时，就可能出现这种情况。更根本的是，个人与设备之间假定的一对一关系往往会失效，特别是在非西方环境中\[403\]。在没有敏感群体成员数据的情况下，测量和纠正不公平是联邦学习研究人员需要解决的关键领域。
...
其他方法包括重新定义现有的公平概念，这些概念主要关注平衡一个结果的可能性\(对受影响的个体来说，一个结果被认为是“积极的”，另一个是“消极的”\)。相反，没有敏感属性的公平可能被重新定义为对有效模型的平等获取。在这种公平的解释下，目标是在所有个体中最大化模型效用，而不考虑他们的\(未知的\)人口统计身份，也不考虑个体结果的“善良”。这匹配的上下文联合学习是最常用的，比如语言建模或医学图像分类,在没有明确的概念的一个结果是“好”的用户,而目的只是为用户做出正确的预测,不管结果如何。
...
此外，目前的差异化私有优化方案没有考虑敏感属性——从这个角度来看，可以预期实证研究表明，差异化私有优化对少数子群体的影响最大。对明确寻求在少数子组上保持性能的差异化私有优化算法的修改，例如，通过调整噪声和剪切机制来考虑数据中组的表示，也可能在很大程度上限制差别隐私建模对受差别隐私训练的联邦模型中的少数子组的潜在差别影响。然而，以一种提供某种形式的隐私保证的方式实现这种自适应差异私有机制，提出了算法和理论上的挑战，需要在未来的工作中解决。
...
在经典的集中式机器学习场景中，过去十年在训练公平分类器方面取得了大量进展，如约束优化、移位后方法和分布鲁棒优化\[223,503,225\]。这些方法已经证明了在提高集中培训的公平性方面的效用，它们是否可以在联邦学习的设置下使用，这是一个有待解决的问题\(如果可以，在何种附加假设下\)，其中数据以分散的方式存在，从业人员可能无法获得符合人口分布的无偏数据样本。
