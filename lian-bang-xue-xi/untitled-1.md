...
**本地差分隐私** 通过让每个客户机在与服务器共享数据之前对其数据应用差分隐私转换，可以在不需要信任集中式服务器的情况下实现差异隐私。也就是说，我们将公式\(3\)应用于处理单个用户的本地数据集D的机制A，并对任何可能的其他本地数据集D'保持保证。该模型被称为本地差分隐私\(LDP\)\[475,266\]。LDP已经被有效地部署在谷歌、苹果和微软的大用户群中收集流行物品的统计数据\[177,154,155\]。它还被Snap\[378\]用于垃圾分类器训练的联邦场景中。这些LDP部署都涉及到大量的客户端和报告，在Snap的情况下甚至多达10亿个，这与集中式的DP实例化形成了鲜明的对比，而DP可以从更小的数据集提供更高的实用性。不幸的是，正如我们将在第4.4.2节中讨论的那样，在维护实用性的同时实现LDP是困难的\[266,455\]。因此，需要一个在纯粹的中心DP和纯粹的局部DP之间插入的差分隐私模型。这可以通过分布式差异隐私或混合模型来实现，如下所述。
...
* 以前研究动态数据库差异隐私的具体方法\[142\]或秘密检测时间序列数据的变化\[143,97\]是否可以扩展到联邦场景？
...
在前一节中，我们假设存在一个可以编排培训过程的可信服务器。在本节中，我们将讨论对敌对服务器进行保护的更理想的场景。特别是，我们从研究这种设置和现有工作的挑战开始，然后描述开放的问题，以及如何使用第4.2节中讨论的技术来解决这些挑战。
...
在没有可信服务器的情况下，可以使用分布式差异隐私\(见第4.2.2节\)来保护参与者的隐私。
...
一些形式的局部和分布式差异隐私也在这里提出了挑战，因为噪声经常被添加到向量的所有元素中，甚至那些是零;因此，在每个客户机上添加这个噪声会将稀疏的模型更新\(即仅在子模型上非零\)转换为密集的私有模型更新\(几乎所有地方都是非零的，且概率很高\)。这种紧张是否能够解决是一个悬而未决的问题，也就是说，是否存在分布式差异隐私的有意义的实例化，它也保持了模型更新的稀疏性。
...
这种方法在Pufferfish隐私框架中形式化\[271\]，它允许分析者指定一类受保护的谓词，这些谓词必须在差异隐私的保证下学习，而所有其他谓词都可以在没有差异隐私的情况下学习。为了在实践中提供令人满意的隐私保障，分析人员必须了解用户对其特定分析任务和数据收集程序的隐私需求。可以修改联邦学习框架，允许单个用户指定他们允许和不允许什么推断。这些数据限制既可以在设备上进行处理，在FL模型更新步骤中只允许与服务器共享信息，也可以在收集数据后作为聚合步骤的一部分进行处理。应进一步开展工作，以开发技术工具，将这种用户偏好纳入FL模型，并开发技术，从用户那里获取有意义的偏好。
...
* 保护用户数据的隐私需要考虑计算数据的哪些功能以及如何执行计算\(特别是谁可以看到/影响中间结果\)。\(4.2节\)
  * 解决“什么”的技术包括数据最小化和差异隐私。\(4.2.2 4.3.2节\)。如何最好地适应不同的隐私审计和私有化技术以适应现实世界的部署仍然是一个重要的开放式挑战，包括由多个独立的参与者在重叠人口上训练大量的机器学习模型，使用随着时间变化的数据，在现实世界的不确定性环境中，如客户可用性，所有这些都不会迅速耗尽隐私预算，同时保持高实用性。
  ...
  * 分布式差异隐私最好地结合了在诚实但好奇的服务器、可信的第三方或可信的执行环境下提供高精度和高隐私性的技术。\(4.2.2,4.4.3节\)
...
联邦场景中训练时间对手的其他维度是他们的适应性。在标准的分布式数据中心训练过程中，恶意数据提供者通常仅限于静态攻击，即在培训开始之前提供一次有毒数据。相比之下，一个能够持续参与联邦场景的恶意用户可以在整个模型训练过程中发起中毒攻击，随着训练的进行，用户可以自适应地修改训练数据或模型更新。请注意，在联邦学习中，这种适应性通常只有在客户端可以在整个训练过程中参与多次时才会感兴趣。
...
针对训练时间和推理时间相结合的对手，一种可能的防御方法是检测后门攻击\[454,108,465,122\]。在第5.1.3节中更详细地讨论了在联邦场景中应用前面的防御\(如上面提到的那些\)的困难。然而，在许多联邦场景中，单纯检测后门可能是不够的，因为我们希望在推断时输出模型的健壮性保证。更复杂的解决方案可能会将训练时间防御\(如健壮的聚合或差异隐私\)与对抗性训练结合起来。该领域的其他开放工作可能涉及量化各种类型的训练时间攻击如何影响模型的推理时间漏洞。考虑到现有的防御纯训练时间攻击或纯推理时间攻击的挑战，这一系列的工作必然更具有思考性和未探索性。
...
**防御模型更新投毒攻击** 服务提供者可以通过\(1\)对客户端模型更新实施规范约束\(例如，剪切客户端更新\)，\(2\)聚合剪切后的更新，\(3\)向聚合中添加高斯噪声来约束任何单个客户端对整体模型的贡献。这种方法防止了对任何个人更新\(或一小群恶意个人\)的过度拟合，并且与差分隐私训练相同\(在第4.3.2节中讨论\)。Sun等人最近研究了这种方法\[438\]，在应用差异隐私防范针对性攻击方面取得了初步成功。然而，Sun等人\[438\]分析的实验和针对性攻击的范围应该扩大到包括更一般的对抗性攻击。特别是，Wang等人\[466\]表明，使用边缘情况后门\(由底层分布中的低概率数据样本生成\)能够绕过差分隐私防御。他们进一步证明了对抗性例子的存在意味着边缘情况下后门的存在，表明对这两种威胁的防御可能需要协同发展。因此，DP是否确实是一种有效的防御还需要更多的工作来验证。更重要的是，它仍然不清楚如何为DP的超参数\(如 l2 范数界限的大小和噪声方差\)可以选择作为模型大小和体系结构的函数，以及恶意设备的比例。
...
虽然差分隐私是一种针对数据中毒的灵活防御，但它也有一些缺点。它的主要缺点是必须在学习过程中注入噪声。虽然这并不一定是一个问题——像随机梯度下降这样的普通学习算法已经注入了噪声——附加的噪声会损害所学习模型的性能。此外，对手只能控制少数设备。因此，差异隐私可以被视为针对数据中毒的一种强防御和一种弱防御——它的强大之处在于它是非常通用的，无论对手的目标是什么，它都能提供最坏的情况保护;它的薄弱之处在于必须限制对手并向联邦学习过程中添加噪声。

**防范推理时间规避攻击** 差异隐私也被研究作为一种防御推理时间攻击，在推理时间攻击中，对手可以修改测试示例来操纵学习的模型。一个简单的方法是使预测器本身具有不同的私密性;然而，这有一个缺点，即预测变得随机化，这是一个通常不受欢迎的特性，也会损害可解释性。更复杂的方法\[296\]添加噪声，然后以最高概率发布预测。我们认为，在这方面还有其他进一步探索的机会。
